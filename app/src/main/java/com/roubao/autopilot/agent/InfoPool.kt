...
data class Action(
    ...
    companion object {
         ...
         * 解析标准 JSON 格式
         * 支持两种格式:
         * 1. 扁平格式: {"action": "click", "coordinate": [x, y]}
         * 2. 嵌套格式: {"action": {"action": "click", "coordinate": [x, y]}, "description": "..."}
         */
        private fun fromStandardJson(json: String): Action? {
            return try {
                val obj = JSONObject(json)

                // 检查是否是嵌套格式 (action 字段是一个对象)
                val actionObj = if (obj.optJSONObject("action") != null) {
                    obj.optJSONObject("action")
                } else {
                    obj
                }

                val type = actionObj.optString("action", "")

                Action(
                    type = type,
                    x = actionObj.optJSONArray("coordinate")?.optInt(0),
                    y = actionObj.optJSONArray("coordinate")?.optInt(1),
                    x2 = actionObj.optJSONArray("coordinate2")?.optInt(0),
                    y2 = actionObj.optJSONArray("coordinate2")?.optInt(1),
                    text = actionObj.optString("text", null),
                    button = actionObj.optString("button", null),
                    duration = if (actionObj.has("duration")) actionObj.optInt("duration", 3) else null,
                    message = actionObj.optString("message", null),
                    needConfirm = actionObj.optBoolean("need_confirm", false),
                    direction = actionObj.optString("direction", null),
                    status = actionObj.optString("status", null)
                )
            } catch (e: Exception) {
                null
            }
