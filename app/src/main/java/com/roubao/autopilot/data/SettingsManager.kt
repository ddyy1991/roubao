...
data class ApiProvider(
    ...
    companion object {
        ...
        val ALL = listOf(GLM_V4_6,GLM_Mobile,GUI_OWL, MAI_UI, ALIYUN, OPENAI, OPENROUTER,CUSTOM)
    }
}
...
data class AppSettings(
    val currentProviderId: String = ApiProvider.GLM_V4_6.id,  // 当前选中的服务商
    ...
    val currentProvider: ApiProvider
        get() = ApiProvider.ALL.find { it.id == currentProviderId } ?: ApiProvider.GLM_V4_6
...
class SettingsManager(context: Context) {
    ...
    private fun loadSettings(): AppSettings {
        ...
        // 加载当前选中的服务商
        val currentProviderId = prefs.getString("current_provider_id", ApiProvider.GLM_V4_6.id) ?: ApiProvider.GLM_V4_6.id
    ...
    private fun loadProviderConfig(providerId: String): ProviderConfig {
        ...
        val storedApiKey = securePrefs.getString("${prefix}api_key", "") ?: ""

        // GLM 默认 API Key
        val defaultApiKey = when (providerId) {
            "glm", "glm_mobile" -> "0ed8d919abbd4fdca81a62ff2cde379f.vaS6vGNeRubgDuiO"
            else -> ""
        }

        return ProviderConfig(
            apiKey = if (storedApiKey.isEmpty()) defaultApiKey else storedApiKey,
