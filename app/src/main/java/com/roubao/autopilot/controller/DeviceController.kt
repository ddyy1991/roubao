...
class DeviceController(private val context: Context? = null) {
    ...
    fun openApp(appNameOrPackage: String) {
        ...
        if (appNameOrPackage.contains(".")) {
            ...
            if (searchResults.isNotEmpty()) {
                ...
                println("[DeviceController] AppScanner found: ${searchResults[0].app.appName} -> $finalPackage (score: ${searchResults[0].score}, type: ${searchResults[0].matchType})")
            } else {
                // 第一次搜索没找到，进行更宽松的模糊匹配
                println("[DeviceController] 首次搜索未找到，尝试模糊匹配: $appNameOrPackage")

                // 扩大搜索范围到 topK=10，并降低匹配阈值
                val expandedResults = appScanner.searchApps(appNameOrPackage, topK = 10)

                if (expandedResults.isNotEmpty()) {
                    finalPackage = expandedResults[0].app.packageName
                    println("[DeviceController] 模糊匹配成功: ${expandedResults[0].app.appName} -> $finalPackage (score: ${expandedResults[0].score}, type: ${expandedResults[0].matchType})")
                } else {
                    // 还没找到，直接遍历所有应用进行模糊匹配
                    println("[DeviceController] 扩大搜索仍未找到，尝试直接遍历匹配")
                    val allApps = appScanner.getApps()
                    val matchedApp = findFuzzyMatch(allApps, appNameOrPackage)

                    if (matchedApp != null) {
                        finalPackage = matchedApp.packageName
                        println("[DeviceController] 模糊遍历匹配成功: ${matchedApp.appName} -> $finalPackage")
                    } else {
                        // 找不到，直接用原始输入尝试
                        finalPackage = appNameOrPackage
                        println("[DeviceController] ⚠️ 未找到应用，使用原始输入尝试: $appNameOrPackage")
                        println("[DeviceController] 提示: 共有 ${allApps.size} 个应用，可检查应用名是否正确")
                    }
                }
            }
        }
     ...
     * 匹配规则：
     * 1. 应用名包含查询词（忽略大小写）
     * 2. 查询词包含应用名的一部分（至少2个字符）
     * 3. 拼音首字母匹配
     * 4. 包名包含查询词
     */
    private fun findFuzzyMatch(apps: List<com.roubao.autopilot.controller.AppScanner.AppInfo>, query: String): com.roubao.autopilot.controller.AppScanner.AppInfo? {
        ...
        val matches = apps.mapNotNull { app ->
            ...
            if (app.appName.lowercase().contains(lowerQuery)) {
                // 根据匹配程度给分
                val matchRatio = lowerQuery.length.toDouble() / app.appName.length
                score = when {
                    matchRatio > 0.8 -> 1.0  // 查询词占应用名80%以上
                    matchRatio > 0.5 -> 0.9  // 查询词占应用名50%以上
                    else -> 0.7              // 包含查询词
                }
                reason = "应用名包含"
            }
            // 2. 查询词包含应用名的一部分（用于用户输入完整应用名的情况）
            else if (lowerQuery.contains(app.appName.lowercase()) && app.appName.length >= 2) {
                score = 0.95
                reason = "包含应用名"
            }
            // 3. 包名包含查询词
            ...
            // 5. 关键词匹配
            else if (app.keywords.any { it.lowercase().contains(lowerQuery) }) {
                score = 0.4
                reason = "关键词匹配"
            }
            ...
            if (score > 0) {
                // 非系统应用加分
                val adjustedScore = if (!app.isSystem) score + 0.1 else score
                Triple(app, adjustedScore.coerceAtMost(1.0), reason)
            } else null
        }
        ...
        if (bestMatch != null) {
            println("[DeviceController] 模糊匹配候选: ${bestMatch.first.appName} (分数: ${bestMatch.second}, 原因: ${bestMatch.third})")
        }
